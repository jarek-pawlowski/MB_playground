...
def example_exact_diagonalization(L, Jz):
    ...
    product_state = (["up", "down"] * L)[:L]  # this selects a charge sector!
    psi_DMRG = MPS.from_product_state(M.lat.mps_sites(), product_state, dtype=complex)
    ...
    rho = psi_ED_mps.get_rho_segment(range(L)).to_ndarray().reshape(2**L,2**L)
    print(np.around(rho, 3))

if __name__ == "__main__":
    example_exact_diagonalization(3, 1.)